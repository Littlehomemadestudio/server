...
TECH_TREE = {
    ...
    "nuclear_weapons": {"name": "تسلیحات هسته ای", "cost": 12000000, "required_level": 18, "prerequisites": ["nuclear_technology", "ballistic_missiles"]},
}
...
async def handle_command(message, command, chat_id, user_id):
    try:
        if command == "/start":
            await message.reply("🤖 ربات فعال!\n\n🌍 برای شروع بازی:\n/countries - مشاهده کشورها\n/select_country [کد] - انتخاب کشور\n/help - راهنما")
        elif command == "/help":
            await message.reply("🤖 راهنما:\n/countries - کشورها\n/select_country [کد] - انتخاب کشور\n/income - درآمد\n/buy [نوع] [تعداد] - خرید\n/military - ارتش\n/tech - فناوری\n/research [tech] - تحقیق")
        elif command == "/countries":
            await show_countries(message)
        elif command.startswith("/select_country"):
            await select_country(message, user_id)
        elif command == "/income":
            await collect_income(message, user_id)
        elif command == "/military":
            await show_military(message, user_id)
        elif command == "/tech":
            await show_tech(message, user_id)
        elif command.startswith("/buy"):
            await buy_asset(message, user_id)
        elif command.startswith("/research"):
            await research_tech(message, user_id)
        else:
            await message.reply("🚫 دستور نامعتبر! /help برای راهنما")
    except Exception as e:
        print(f"Command error: {e}")

# -------------------- COMMAND FUNCTIONS --------------------
async def show_countries(message):
    text = "🌍 کشورهای موجود:\n\n"
    for code, country in AVAILABLE_COUNTRIES.items():
        text += f"**{country['name']}**\n"
        text += f"💰 پول شروع: ${country['starting_money']:,}\n"
        text += f"🔑 کد: {code}\n\n"
    text += "برای انتخاب: /select_country [کد]"
    await message.reply(text)

async def select_country(message, user_id):
    ...
    if user_data.get("country_selected", False):
        await message.reply("⚠️ قبلاً کشور انتخاب کرده‌اید!")
        return
    
    if country_code not in AVAILABLE_COUNTRIES:
        await message.reply("⚠️ کشور نامعتبر!")
        return
    ...
    await message.reply(f"🎉 خوش آمدید به {country_data['name']}!\n💰 پول شروع: ${country_data['starting_money']:,}")

async def collect_income(message, user_id):
    user_data = get_global_user_data(user_id)
    
    if not user_data.get("country_selected", False):
        await message.reply("⚠️ ابتدا کشور انتخاب کنید!")
        return
    
    current_time = time.time()
    if current_time - user_data.get("last_income_time", 0) < user_data.get("income_cooldown", 3600):
        remaining = int(user_data.get("income_cooldown", 3600) - (current_time - user_data.get("last_income_time", 0)))
        await message.reply(f"⏳ {remaining // 60} دقیقه صبر کنید!")
        return
    
    country_data = AVAILABLE_COUNTRIES[user_data["country"]]
    total_income = 0
    
    for method_id, method_data in country_data['income_methods'].items():
        income = int(method_data['base_income'] * method_data['multiplier'])
        total_income += income
    ...
    await message.reply(f"💰 درآمد: ${total_income:,}\n💎 موجودی: ${user_data['points']:,}")

async def show_military(message, user_id):
    user_data = get_global_user_data(user_id)
    text = "🎖 ارتش شما:\n\n"
    total_power = 0
    
    for asset_id, count in user_data["military"].items():
        if count > 0:
            asset = MILITARY_ASSETS[asset_id]
            power = asset["power"] * count
            total_power += power
            text += f"• {asset['name']}: {count} (قدرت: {power})\n"
    
    if total_power == 0:
        text += "هیچ تجهیزی ندارید!"
    else:
        text += f"\n💪 قدرت کل: {total_power}"
    
    await message.reply(text)

async def show_tech(message, user_id):
    user_data = get_global_user_data(user_id)
    text = "🔬 فناوری:\n\n"
    
    for tech_id, tech in TECH_TREE.items():
        status = "✅" if tech_id in user_data["technologies"] else "🔒"
        text += f"{status} {tech['name']} - ${tech['cost']:,}\n"
    
    text += "\n/research [tech_id] برای تحقیق"
    await message.reply(text)

async def buy_asset(message, user_id):
    parts = message.content.split()
    if len(parts) < 3:
    ...
    try:
        ...
        await message.reply("⚠️ تعداد نامعتبر!")
        return
    ...
    asset = MILITARY_ASSETS[asset_type]
    cost = asset["cost"] * quantity
    
    if user_data["points"] < cost:
        await message.reply(f"⚠️ نیاز: ${cost:,}")
        return
    ...
    await message.reply(f"✅ {quantity} {asset['name']} خریداری شد!\n💰 باقی: ${user_data['points']:,}")

async def research_tech(message, user_id):
    parts = message.content.split()
    if len(parts) < 2:
    ...
    if user_data["points"] < tech["cost"]:
        await message.reply(f"⚠️ نیاز: ${tech['cost']:,}")
        return
